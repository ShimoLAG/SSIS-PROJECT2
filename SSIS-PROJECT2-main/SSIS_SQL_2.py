...
class App(Tk):
    ...
    def editcourse_confirm(self, courseedit, ccode, cname, course_window, initial_ccode):
        try:
            ...
            new_ccode = ccode.get().strip()
            new_cname = cname.get().strip()

            if not new_ccode or not new_cname:
            ...
            check_query = "SELECT COUNT(*) FROM Course WHERE coursecode = %s AND coursecode != %s"
            ...
            if result[0] > 0:
                ...
                try:
                    ...
                    update_course_query = "UPDATE Course SET coursecode = %s, course_name = %s WHERE coursecode = %s"
    ...
    def get_input(self):
        ...
        try:
            mycursor.execute(sql_query, x)  # Pass individual elements of x
            db.commit()
        except mysql.connector.errors.IntegrityError as e:
            # Check if the error is due to a duplicate entry
            if 'Duplicate entry' in str(e):
                # Display an error message indicating that the ID already exists
                showwarning("Duplicate Entry", "A student with the same ID already exists.")
            else:
                # For other IntegrityError types, display a generic error message
                showwarning("Database Error", "An error occurred while adding the student to the database.")
